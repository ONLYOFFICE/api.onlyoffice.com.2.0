...
export class TocContainer extends HTMLElement {
  static get tagName(): "toc-container" {
  ...
  #defaultRootMargin = "-0px 0px 0px 0px"
  ...
  set rootMargin(s: string | null) {
    if (s === null) {
      s = this.#defaultRootMargin
    }
    this.#rootMargin = s
  }
  ...
  get #targetQuery(): string {
    ...
    let q = "h1, h1 ~ *:not(section), "

    for (const e of s) {
      const id = e.hash.slice(1)
      q += `h2[id="${id}"], h2[id="${id}"] ~ *:not(section), `
    }
  ...
  get #targetMap(): Map<Element, string> {
