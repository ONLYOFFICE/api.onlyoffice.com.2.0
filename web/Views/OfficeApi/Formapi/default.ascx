...
builder.SaveFile("form", "example.pdf");       // save the resulting form document as a file in the form format with the 'example.pdf' name
builder.CloseFile();                              // close the form document file and finish work with ONLYOFFICE Document Builder</pre>
    <p>As you can see you just need to use the <em>builder.OpenFile();</em> method of the <a href="<%= Url.Action("integrationapi/cdocbuilder", "docbuilder") %>">CDocBuilder</a> class with the path to the necessary form document as an argument to open it. In the above example we open <b>myformdocument.docx</b> document, get its first paragraph and add the text form with the "John Smith" text to it. The same way any other form document element can be changed.</p>
